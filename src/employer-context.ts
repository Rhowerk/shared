...
export interface WerteDef {
  ...
  label: string
  icon: string
  prompt_hint: string // Hilft der AI den Wert zu konkretisieren
}

export const WERTE_CATALOG: WerteDef[] = [
  {
    id: 'augenhoehe',
...
export function getWert(id: string): WerteDef | undefined {
  return WERTE_CATALOG.find(w => w.id === id)
}

export function getWerteLabels(ids: string[]): string[] {
  return ids.map(id => getWert(id)?.label || id)
}

// ── Employer Context pro Fachrichtung ──

export interface EmployerContext {
  fachrichtung: string
  typisch: string
  aufgaben: Record<string, string>  // Rolle → Aufgabenbeschreibung
  staerke: string                   // USP als Arbeitgeber
  benefit_kontexte: Record<string, string> // benefit_id → fachrichtungsspezifisch
  verbotene_phrasen: string[]
}

const EMPLOYER_CONTEXTS: Record<string, EmployerContext> = {
  allgemeinmedizin: {
...
const FALLBACK_CONTEXT: EmployerContext = {
...
export function getEmployerContext(fachrichtung: string): EmployerContext {
...
export function getAvailableFachrichtungen(): string[] {
