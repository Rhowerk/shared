...
export const BENEFIT_CATEGORIES: BenefitCategoryDef[] = [
...
export function getBenefitsByCategory(): Record<BenefitCategory, BenefitItem[]> {
  const grouped: Record<BenefitCategory, BenefitItem[]> = {
    verguetung: [],
    arbeitszeit: [],
    entwicklung: [],
    extras: [],
...
export function getBenefitLabels(ids: string[]): string[] {
  return ids.map(id => {
...
export function validateBenefits(ids: string[]): { known: string[]; custom: string[] } {
...
export function getBenefit(id: string): BenefitItem | undefined {
