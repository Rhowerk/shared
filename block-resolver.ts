...
export interface ResolvedBlock {
  collection: string      // Directus Collection: 'block_imageText', 'block_herobanner', etc.
  role: BlockRole         // Semantische Rolle → bestimmt Surface-Zuweisung
  variant: string         // Darstellungsvariante im Template (z.B. 'grid-2', 'text-only')
  contentKey: string      // Schlüssel für AI-Content-Zuordnung
  label: string           // Dashboard-Anzeigename (praxisnah, kein Tech-Jargon)
  required: boolean       // Pflichtblock oder datenabhängig?
  condition?: string      // Debug/Dashboard: Warum dieser Block da ist
}
...
export interface BlockResolverInput {
  ...
  // Feature-Flags (aus Wizard-Daten)
  hasTeam: boolean
  teamCount: number
  hasTeamPhotos: boolean
  hasKarriere: boolean
  hasBildmaterial: boolean    // Hat der User Bilder hochgeladen?

  // Leistungen
  serviceCount: number

  // Karriere / Employer Branding (optional)
  benefits?: string[]         // Benefit-IDs
  benefitCount?: number
  werte?: string[]            // Werte-IDs
  werteFreitext?: string      // "Jeden Freitag kochen wir zusammen"
 ...
 */
export function extractResolverInput(data: Record<string, any>): BlockResolverInput {
  ...
  if (Array.isArray(eb.benefits?.selected)) {
    ...
    benefits = eb.benefits.split(/[,;\n]+/).map((b: string) => b.trim()).filter((b: string) => b.length > 2)
  }
...
export function resolveHomepageBlocks(input: BlockResolverInput): ResolvedBlock[] {
...
export function resolveKarriereBlocks(input: BlockResolverInput): ResolvedBlock[] {
  const blocks: ResolvedBlock[] = []
...
export function resolveTeamBlocks(input: BlockResolverInput): ResolvedBlock[] {
  const blocks: ResolvedBlock[] = []
...
export function resolveServiceDetailBlocks(input: BlockResolverInput): ResolvedBlock[] {
  return [
    {
      collection: 'block_herobanner',
      role: 'hero',
...
export function resolveJobDetailBlocks(input: BlockResolverInput): ResolvedBlock[] {
  const benefitCount = input.benefitCount || 0
