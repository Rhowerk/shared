...
// ═══════════════════════════════════════════════════════════
import { getEmployerContext, getWert } from './employer-context.js';
import { getBenefitLabels } from './benefit-catalog.js';
// ── Tonalitäts-Instruktionen ──
export const TONALITY_INSTRUCTIONS = {
...
export function buildBlockPrompt(req) {
...
${ec.verbotene_phrasen.length > 0 ? `\nVERBOTEN: ${ec.verbotene_phrasen.map(p => `"${p}"`).join(', ')}` : ''}`;
    ...
    if (req.blockSpecific) {
        ...
        if (req.blockSpecific.werte) {
            const werteDetails = req.blockSpecific.werte.map((id) => {
                const w = getWert(id);
...
export function buildBatchPrompt(blocks, context, blockData) {
...
${ec.verbotene_phrasen.length > 0 ? `\nVERBOTEN: ${ec.verbotene_phrasen.map(p => `"${p}"`).join(', ')}` : ''}`;
    ...
    for (const block of blocks) {
        ...
        if (specific?.werte) {
            const werteDetails = specific.werte.map((id) => {
                const w = getWert(id);
                return w ? `${w.label} (${w.prompt_hint})` : id;
            });
            part += `\nWerte: ${werteDetails.join('; ')}`;
...
export function mapTonalityToPreset(tonality) {
    ...
    if ((tonality.atmosphere || 3) <= 2 && (tonality.language || 3) <= 2)
        return 'herzlich';
    // sachlich + fachlich → fachlich
    if ((tonality.atmosphere || 3) >= 4 && (tonality.language || 3) >= 4)
        return 'fachlich';
    // modern + locker → entspannt
    if ((tonality.style || 3) <= 2 && (tonality.demeanor || 3) <= 2)
        return 'entspannt';
    // klassisch + seriös → verlässlich
    if ((tonality.style || 3) >= 4)
        return 'verlaesslich';
