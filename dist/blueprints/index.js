 ...
 */
export function getBlueprint(fachrichtung) {
    const normalized = fachrichtung.toLowerCase().trim();
    // Direkt vorhanden?
    if (ALL_BLUEPRINTS[normalized])
        return ALL_BLUEPRINTS[normalized];
    // Fallback über Registry
    const fallbackId = getBlueprintId(normalized);
    if (fallbackId && ALL_BLUEPRINTS[fallbackId])
        return ALL_BLUEPRINTS[fallbackId];
    return null;
}
/**
 * Gibt alle verfügbaren Blueprint-IDs zurück.
 */
export function getAvailableBlueprintIds() {
 ...
 */
export function getBlueprintServices(blueprint) {
 ...
 */
export function getServicesByCategory(blueprint) {
    if (!blueprint?.services)
        return [];
 ...
 */
export function findService(blueprint, name) {
    if (!blueprint?.services)
        return null;
 ...
 */
export function generateSEO(blueprint, data) {
 ...
 */
export function buildAIContext(blueprint, data) {
    if (!blueprint) {
    ...
    if (ctx.language_style) {
        prompt += `\nSPRACHSTIL:\n`;
        if (ctx.language_style.avoid.length)
            prompt += `- Vermeide: ${ctx.language_style.avoid.join(', ')}\n`;
        if (ctx.language_style.prefer.length)
            prompt += `- Bevorzuge: ${ctx.language_style.prefer.join(', ')}\n`;
    }
    if (emp.typisch) {
 ...
 */
export function buildServicePrompt(blueprint, serviceName) {
 ...
 */
export function getEmployerData(fachrichtung) {
...
export function getAufgaben(fachrichtung, rolle) {
    ...
    // Direct match
    if (aufgaben[lower])
        return aufgaben[lower];
    // Fuzzy match
    if (lower.includes('mfa') || lower.includes('medizinische fachangestellte'))
        return aufgaben['mfa'] || aufgaben['zfa'] || '';
    if (lower.includes('zfa') || lower.includes('zahnmedizinische'))
        return aufgaben['zfa'] || aufgaben['mfa'] || '';
    if (lower.includes('arzt') || lower.includes('ärztin') || lower.includes('facharzt'))
        return aufgaben['arzt'] || '';
    if (lower.includes('azubi') || lower.includes('auszubildend'))
        return aufgaben['azubi'] || '';
    if (lower.includes('mtra') || lower.includes('radiolog'))
        return aufgaben['mtra'] || aufgaben['mfa'] || '';
    if (lower.includes('therapeut'))
        return aufgaben['therapeut'] || aufgaben['mfa'] || '';
 ...
 */
export function getBlueprintTeamPositions(fachrichtung) {
    const bp = getBlueprint(fachrichtung);
    const positions = [];
    // ── Ärzte-Positionen ──
