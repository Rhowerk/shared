 ...
 */
export function assignSurfaces(blocks, theme, dividerType = 'none') {
    const count = blocks.length;
    // 1. Surface pro Block bestimmen
    const surfaces = blocks.map((block, i) => {
 ...
 */
export function getBlockSurfaceConfig(v2Data) {
    const theme = v2Data || {
        seed: {
     ...
     */
    function forBlock(collection, sortIndex, totalBlocks, nextCollection) {
...
function getSurfaceForBlockLegacy(role, sortIndex, totalBlocks, surfaces) {
    if (role === 'hero')
